...
def train_one_model(
    ...
    metrics = compute_regression_metrics(np.array(y_true), np.array(y_pred))
    return {
...
def compute_regression_metrics(
    y_true: np.ndarray, y_pred: np.ndarray, huber_delta: float = 1.0, quantile: float = 0.5
) -> Dict[str, float]:
    error = y_true - y_pred
    ...
    log_cosh = float(np.mean(np.log(np.cosh(error))))
    quantile_loss = float(np.mean(np.maximum(quantile * error, (quantile - 1.0) * error)))
    # Mean Bias Error positif menandakan prediksi cenderung overestimate.
...
def train_with_history(
    ...
    regression_loss_history: Dict[str, List[float]] = {
        "mse": [],
        "mae": [],
        "huber_loss": [],
        "log_cosh_loss": [],
    ...
    for epoch in range(1, epochs + 1):
        ...
        model.eval()
        val_batch_losses = []
        val_true_epoch: List[float] = []
        val_pred_epoch: List[float] = []
        with torch.no_grad():
            for xb, yb in test_loader:
                xb, yb = xb.to(device), yb.to(device)
                pred = model(xb)
                val_batch_losses.append(criterion(pred, yb).item())
                val_pred_epoch.extend(pred.cpu().numpy().flatten())
                val_true_epoch.extend(yb.cpu().numpy().flatten())
        epoch_val_loss = float(np.mean(val_batch_losses))

        train_losses.append(epoch_train_loss)
        val_losses.append(epoch_val_loss)
        epoch_reg_metrics = compute_regression_metrics(
            np.array(val_true_epoch), np.array(val_pred_epoch)
        )
        for key in regression_loss_history:
            regression_loss_history[key].append(epoch_reg_metrics[key])
    ...
    regression_metrics = compute_regression_metrics(y_true_np, y_pred_np)
    cls_metrics = compute_classification_metrics(y_true_np, y_pred_np)

    return {
        "regression_metrics": regression_metrics,
        "regression_loss_history": regression_loss_history,
